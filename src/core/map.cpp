...
void Map::deleteSelectedObjects()
{
	...
	if (obj != end)
	{
		...
		for (; obj != end; ++obj)
		{
			...
			if (index >= 0)
			{
				undo_step->addObject(index, part->releaseObject(index));
			}
			else
			{
...
int Map::reassignObjectsToMapPart(std::vector<int>::const_iterator first, std::vector<int>::const_iterator last, std::size_t source, std::size_t destination)
{
	...
	for (auto it = first; it != last; ++it)
	{
		...
		target_part->addObject(source_part->releaseObject(object));
	}
...
int Map::mergeParts(std::size_t source, std::size_t destination)
{
	...
	{
		target_part->addObject(source_part->releaseObject(0));
