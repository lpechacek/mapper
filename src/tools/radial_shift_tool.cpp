...
RadialShiftTool::RadialShiftTool(MapEditorController* editor, QAction* tool_action)
: MapEditorToolBase(scaledToScreen(QCursor{ QPixmap(QString::fromLatin1(":/images/cursor-radial-shift.png")), 1, 1 }),
...
RadialShiftTool::~RadialShiftTool()
{
	// nothing
}


void RadialShiftTool::updateStatusText()
{
...
void RadialShiftTool::objectSelectionChangedImpl()
{
	updateHoverState();
}
 ...
 */
void RadialShiftTool::updateHoverState()
{
	    ...
	    new_hover_object != hover_object)
	{
		...
		if (hover_state == HoverFlag::OverPathEdge)
		{
...
void RadialShiftTool::mouseMove()
{
	updateHoverState();
}


void RadialShiftTool::dragStart()
{
...
void RadialShiftTool::dragFinish()
{
	if (!hover_object)
		return;
...
void RadialShiftTool::drawImpl(QPainter* painter, MapWidget* widget)
{
