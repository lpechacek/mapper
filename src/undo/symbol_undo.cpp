...
SymbolUndoStep::SymbolUndoStep(Map* map)
    : SymbolUndoStep(map, UndefinedChange, nullptr, 0)
{
	// nothing
}


SymbolUndoStep::~SymbolUndoStep()
{
	// nothing
}
...
UndoStep* SymbolUndoStep::undo()
{
	...
	case AddSymbol:
		...
		break;
	case RemoveSymbol:
		redo_step = new SymbolUndoStep(map, AddSymbol, map->releaseSymbol(symbol_pos), symbol_pos);
		break;
